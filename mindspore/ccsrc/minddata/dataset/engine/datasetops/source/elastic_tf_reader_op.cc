...
#include "minddata/dataset/engine/datasetops/source/site_profiler.hpp"
#include <tracer/site>
...
Status ElasticTFReaderOp::Init() {
  // fprintf(stderr, "ElasticTFReaderOp::%s\n", __func__);
...
Status ElasticTFReaderOp::LoadFile(const std::string &filename, int64_t start_offset, int64_t end_offset,
                                   int32_t worker_id) {
  // TRACE_SITE_SCOPE("ElasticTFReaderOp::LoadFile");
  if (true) {
  ...
  while (reader.peek() != EOF) {
    ...
    if (start_offset == kInvalidOffset || (rows_total >= start_offset && rows_total < end_offset)) {
      // TRACE_SITE_SCOPE("new one row");
      dataengine::Example tf_file;
      {  // TRACE_SITE_SCOPE("tf_file.ParseFromString");
      ...
      {  // TRACE_SITE_SCOPE("LoadExample");
        RETURN_IF_NOT_OK(LoadExample(&tf_file, &newRow));
      }
      rows_read++;
      {  // TRACE_SITE_SCOPE("jagged_rows_connector_->Add");
